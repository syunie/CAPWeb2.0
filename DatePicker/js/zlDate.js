...
var obj = { date: new Date(), year: -1, month: -1, priceArr: [], posArr: [] };
var htmlObj = { header: "", left: "", right: "" };
var elemId = null;

//获得地址栏参数，getUrlPara("ID")
...
var pickerEvent = {
    Init: function (elemid) {
        ...
        document.getElementById("calendar_choose").style.zIndex = 1;
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                if (obj.year == new Date().getFullYear() && obj.month == new Date().getMonth() + 1 && i - c == new Date().getDate()) {
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + ' pos="' + pos + '"><a href="#"><span class="date basefix">今天</span><span class="calendar_price01">' + priceStr + '</span><span class="team basefix" >' + posString + '</span></a></td>';
                }
                else {
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + ' pos="' + pos + '"><a href="#"><span class="date basefix">' + (i - c) + '</span><span class="calendar_price01">' + priceStr + '</span><span class="team basefix">' + posString + '</span></a></td>';
                }
                if (index == 6) {
...
var commonUtil = {
    ...
    getPos: function (day) {
        ...
        for (var i = 0; i < obj.posArr.length; i++) {
            if (obj.posArr[i].Date == dt) {
                return obj.posArr[i].Pos.split('.')[0];
            }
        }
        return -1;
    },
    chooseClick: function (sender) {
        ...
        if (el != null) {
            ...
            if (promotionID == "") {
                ...
                window.location.href = "UrlRouter.aspx?Type=Line&ID=" + id + "&date=" + date;
            }
            else {
                var id = getUrlPara("id");
                var lineID = getUrlPara("LineID");
                //var url = "UrlRouter.aspx?Type=Promotion&ID=" + id + "&PromotionID=" + promotionID + "&LineID=" + lineID + "&date=" + date;
                window.location.href = "UrlRouter.aspx?Type=Promotion&ID=" + id + "&PromotionID=" + promotionID + "&LineID=" + lineID + "&date=" + date;
            }
            //alert("日期是："+date);
            //alert("价格是：￥"+price);
            //var lineID = getUrlPara("LineID");
